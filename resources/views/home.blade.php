...
<div id="chatbot-container" class="fixed bottom-6 right-6 z-50 flex flex-col items-end">
    <div id="chat-window" class="hidden w-80 sm:w-96 h-[500px] max-h-[80vh] bg-white rounded-2xl shadow-2xl border border-gray-200 flex flex-col mb-4 overflow-hidden origin-bottom-right transition-all duration-300">
        ...
        <div id="chat-messages" class="flex-1 overflow-y-auto overflow-x-hidden p-4 bg-gray-50 flex flex-col gap-3 text-sm border-t border-b border-gray-100 min-h-0">
            <div class="bg-blue-100 text-blue-800 p-3 rounded-2xl rounded-tl-sm w-fit max-w-[85%] shadow-sm break-words">
...
<style>
    /* Ép text và code trong markdown phải rớt dòng, không được tràn ra ngoài tạo thanh cuộn ngang */
    .markdown-body p { margin-bottom: 0.5rem; word-break: break-word; }
    .markdown-body strong { font-weight: 700; color: #1e3a8a; }
    .markdown-body ul { list-style-type: disc; padding-left: 1.5rem; margin-bottom: 0.5rem; }
    .markdown-body pre, .markdown-body code { 
        background-color: #f1f5f9; 
        border-radius: 0.25rem; 
        white-space: pre-wrap !important; /* Ép code rớt dòng */
        word-wrap: break-word !important; 
    }
    .markdown-body pre { padding: 0.5rem; margin-bottom: 0.5rem; }
    .markdown-body code { font-family: monospace; padding: 0.1rem 0.3rem; }
...
<script>
    ...
    function appendMessage(sender, text) {
        ...
        if (sender === 'user') {
            // Bong bóng của user
            div.className = "bg-blue-600 text-white p-3 rounded-2xl rounded-tr-sm w-fit max-w-[85%] self-end shadow-sm break-words";
            div.textContent = text;
        } else {
            // Bong bóng của AI (Đã gỡ bỏ overflow-x-auto, chỉ giữ break-words)
            div.className = "markdown-body bg-blue-100 text-blue-900 p-3 rounded-2xl rounded-tl-sm w-fit max-w-[85%] shadow-sm leading-relaxed break-words";
            div.innerHTML = marked.parse(text); 
        }
        chatMessages.appendChild(div);
        
        // Cuộn toàn bộ box chat xuống dưới cùng
